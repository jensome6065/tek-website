...
/** Profile photos and LinkedIn URLs keyed by preferred roster name. */
...
const profileAliases: Record<string, string> = {
  "Samantha Carroll": "Sam Carroll",
  "Ariana Thomas": "Ari Thomas",
};

export function getMemberProfile(name: string): MemberProfile | undefined {
  const key = profileAliases[name] ?? name;
  return memberProfiles[key];
}
